...
4. **Build safe redirect URL**
   ...
   - Get trusted `SITE_URL` from environment (not from request headers)
...
- [ ] **Callback rate limiting** — `/auth/callback` also rate-limited by IP — [`callback/route.ts:9`](../src/app/auth/callback/route.ts#L9)
